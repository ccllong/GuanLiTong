...
<div class="main-title">
    <h2>
        <?php echo !empty($nav_type)?$nav_type : '顶部'; ?>导航栏
    </h2>
</div>
...
<div class="cf" >
    <div class="fl">
        <a class="btn" href="<?php echo !empty($nav_type)?url('index/addFooterNav') : url('index/addNav'); ?>">添 加</a>
    </div>
</div>
...
<div class="data-table">
    <table class="">
        ...
        <tbody>
        <?php if(!(empty($nav_list) || (($nav_list instanceof \think\Collection || $nav_list instanceof \think\Paginator ) && $nav_list->isEmpty()))): if(is_array($nav_list) || $nav_list instanceof \think\Collection || $nav_list instanceof \think\Paginator): $i = 0; $__LIST__ = $nav_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$nav): $mod = ($i % 2 );++$i;?>
        <tr>
            <td><?php echo $nav['_id']; ?></td>
            <td><?php echo $nav['title']; ?></td>
            <td><?php echo $nav['url']; ?></td>
            <td><?php echo $nav['sort']; ?></td>
            <td><?php echo !empty($nav['target']) && $nav['target']==1?'是' : '否'; ?></td>
            <td><?php echo !empty($nav['status']) && $nav['status']==1?'<font color="red">禁用</font>' : '<font color="green">启用</font>'; ?></td>
            <td><?php echo getDateFromTimestamp($nav['updateAt']); ?></td>
            <td>
                <a href="<?php echo !empty($nav_type)?url('index/editFooterNav',['id'=>$nav['_id']]) : url('index/editNav',['id'=>$nav['_id']]); ?>">编辑</a>
                <a href="<?php echo !empty($nav_type)?url('index/delFooterNav',['id'=>$nav['_id']]) : url('index/delNav',['id'=>$nav['_id']]); ?>" class="confirm ajax-get">删除</a>
            </td>
        </tr>
